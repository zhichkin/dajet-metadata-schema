...
Свойство **delete** кодируется как объект JSON и является необязательным (опциональным).
В случае записи набора в режиме "Замещение", равному значению **Ложь**, свойство **delete**
не используется и не сериализуется в выходной JSON.
Запись набора в режиме "Замещение", равному значению **Истина** (используется в 1С по умолчанию),
без указания отбора означает удаление всех записей соответствующей таблицы регистра.

Свойство **insert** кодируется как массив объектов JSON и является необязательным (опциональным).
Отсутствие свойства **insert** в выходном JSON буквально означает то,
что записей для вставки в целевую таблицу соответствующего регистра нет.
```json
{
   "delete":
   {
...
Сериализация операции удаления записи регистра выполняется аналогично сериализации
объекта "УдалениеОбъекта" для ссылочных типов данных. При этом сериализация значений
ресурсов и реквизитов записи не обязательна - сериализуется только ключ записи.
```json
// Тип сообщения: УдалениеОбъекта или ObjectDeletion
{
   "key":
